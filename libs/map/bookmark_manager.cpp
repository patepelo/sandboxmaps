...
std::string BuildIndexFile(std::vector<std::string> const & filesForIndex)
{
  ...
  std::string content =
      ...
      "<Document>\n"
      "<name>Sandbox Maps Bookmarks and Tracks</name>\n";
...
{
  auto const kmzFileName = "SandboxMaps_backup_" + std::to_string(base::GenerateYYMMDD(time(nullptr)));
  ...
  for (auto const & kmlToExport : *collection)
  {
    ...
    if (!strings::IsASCIIString(fileName))
      fileName = "SandboxMaps_" + std::to_string(suffix++);
